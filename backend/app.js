...
const textToSpeech = new TextToSpeechV1({
   iam_apikey: 'CB1WmWCSzPR1Apl3j49mmpVmo7KiT0w9UzetpstaQ8mM',
});

app.post('/api/speak', function (req, res) {
    console.log(req.body.text);
    var speak = textToSpeech.synthesize(req.body)
        .then(audio => {
            audio.pipe(fs.createWriteStream('speech.mp3'));
        })
        .catch(err => {
            console.log('error:', err);
        });
        setTimeout(function () {
        load('./speech.mp3').then(play);
        }, 1500);
