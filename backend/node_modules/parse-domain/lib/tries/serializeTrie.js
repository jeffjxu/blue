...
function lineToString(line, i, arr) {
  ...
  if (i > 0) {
    ...
    if (indexOfDifference === 0) {
      ...
      var levelsUp = prevLine.length - indexOfDifference - 1;
      /* istanbul ignore if */
      ...
      separatorFromPrev = new Array(levelsUp).fill(SEPARATORS.UP).join("");
    }
  }
...
function serializeTrie(parsedList, type) {
  ...
  if (POSSIBLE_TYPES.indexOf(type) === -1) {
    throw new Error("Cannot serialize trie: Unknown trie type \"".concat(type, "\". Expected type to be one of ").concat(POSSIBLE_TYPES.join(", ")));
  }
